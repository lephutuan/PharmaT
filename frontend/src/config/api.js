...
const API = {
    ...
    try {
      ...
      if (hoaDonData.ma_kh) {
        ...
        if (khachHang && khachHang.loai_kh === "Hội viên") {
          const newTotal =
            (khachHang.tong_tien_da_mua || 0) + hoaDonData.tong_tien;
          await window.supabaseClient
            .from("khach_hang")
  ...
  async getStatistics() {
    try {
      ...
      const totalRevenue =
        monthRevenue?.reduce((sum, hd) => sum + parseFloat(hd.tong_tien), 0) ||
        0;
  ...
  /**
   * Lấy danh sách thuốc bán chạy (top selling) - Sử dụng VIEW
   */
  async getTopMedicines(limit = 5, type = "quantity") {
    try {
      // Sử dụng TABLE để lấy dữ liệu đã được aggregate sẵn (nhanh hơn VIEW)
      const { data, error } = await window.supabaseClient
        .from("table_top_thuoc_ban_chay")
        .select("*")
        .order(type === "quantity" ? "tong_so_luong_ban" : "tong_doanh_thu", {
          ascending: false,
        })
        .limit(limit);
      ...
      return data.map((item) => ({
        ...
        anh: item.anh,
        don_vi: item.don_vi,
        tong_so_luong: item.tong_so_luong_ban,
        tong_doanh_thu: item.tong_doanh_thu,
      }));
    } catch (error) {
      console.error("Lỗi khi lấy thuốc bán chạy:", error);
      return [];
    }
  },
  ...
  async getExpiringMedicines(days = 30) {
    try {
      // Sử dụng TABLE để lấy thuốc sắp hết hạn đã được calculate sẵn
      const { data, error } = await window.supabaseClient
        .from("table_thuoc_sap_het_han")
        .select("*")
        .order("han_su_dung", { ascending: true });

      if (error) throw error;

      // VIEW đã có sẵn muc_do và so_ngay_con_lai
      return data.map((medicine) => ({
        ...medicine,
        days_left: medicine.so_ngay_con_lai,
        status: medicine.muc_do,
      }));
    } catch (error) {
  ...
  async getLowStockMedicines() {
    try {
      // Sử dụng TABLE để lấy thuốc tồn kho thấp đã được filter sẵn
      const { data, error } = await window.supabaseClient
        .from("table_thuoc_ton_kho_thap")
        .select("*")
        .order("so_luong", { ascending: true });

      if (error) throw error;

      // VIEW đã có sẵn status
      return data;
    } catch (error) {
      console.error("Lỗi khi lấy thuốc tồn kho thấp:", error);
      return [];
    }
  },

  /**
   * Lấy phân bố theo nhóm thuốc
   */
  async getMedicineGroups() {
    try {
      const { data, error } = await window.supabaseClient
        .from("thuoc")
        .select("nhom_thuoc");
      ...
      const groups = {};
      data.forEach((thuoc) => {
        const nhom = thuoc.nhom_thuoc || "Khác";
        groups[nhom] = (groups[nhom] || 0) + 1;
      });
  ...
  /**
   * Lấy bảng dữ liệu chi tiết doanh thu
   */
  async getDetailedSales(timeFilter = "month") {
    try {
      const today = new Date();
      let startDate;
      ...
      // Gọi RPC function để lấy daily stats
      ...
      // Transform data
      return data.map((row) => ({
        ngay: new Date(row.ngay).toLocaleDateString("vi-VN"),
        doanh_thu: row.doanh_thu.toLocaleString("vi-VN"),
        so_hoa_don: row.so_hoa_don,
        nv_thuc_hien_nhieu_nhat: row.nv_thuc_hien_nhieu_nhat,
        thuoc_ban_chay_nhat: row.thuoc_ban_chay_nhat,
      }));
    } catch (error) {
  ...
  /**
   * Lấy dữ liệu báo cáo đầy đủ
   */
  async getReportsData(timeFilter = "month") {
    try {
      // Get today date
      const today = new Date();
      let startDate;
      ...
      const { data: invoices, error: invError } = await window.supabaseClient
        .from("hoa_don")
        .select("tong_tien, ngay_lap")
        .gte("ngay_lap", startDate)
        .lte("ngay_lap", endDate)
        .eq("trang_thai", "Đã thanh toán");

      if (invError) throw invError;

      const totalRevenue = invoices.reduce(
        (sum, inv) => sum + parseFloat(inv.tong_tien),
        0
      );
      ...
      const { count: inventoryCount } = await window.supabaseClient
        .from("thuoc")
        .select("*", { count: "exact", head: true })
        .neq("trang_thai", "Hết hạn");

      // Get expiring medicines
      const expiringMeds = await this.getExpiringMedicines(30);

      // Get medicine groups
      const medicineGroups = await this.getMedicineGroups();
      ...
      return {
        ...
        alerts: {
          expiring: expiringMeds.slice(0, 3),
          lowStock: lowStock.slice(0, 3),
        },
        detailTable: await this.getDetailedSales(timeFilter),
      };
    } catch (error) {
